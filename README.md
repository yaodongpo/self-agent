...
上传服务：
- `--upload-endpoint http://.../upload` 或 `SELF_AGENT_UPLOAD_ENDPOINT`
- `--upload-api-key TOKEN` 或 `SELF_AGENT_UPLOAD_API_KEY`
...
- 视觉：capture_screen（返回图片 data url 或上传后的 url）
- 结构化评估器：
...
豆包/Ark Responses 通常要求 `image_url` 为可访问的 URL；因此当你使用 Ark 模型且要发截图/图片时，建议提供上传服务并把图片转换成 URL。
