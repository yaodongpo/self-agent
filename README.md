...
- `--image-transport inline|upload|auto`：图片传输方式（默认 inline）
- `--no-memory`：禁用记忆读写与压缩
...
- `--upload-api-key TOKEN` 或 `SELF_AGENT_UPLOAD_API_KEY`（可选）
  ...
  - 当 image-transport=upload/auto 时，需要配置 upload-endpoint 才能返回可访问 URL
...
- UI 观测：ui\_get\_cursor\_pos / ui\_get\_screens / ui\_get\_active\_window
